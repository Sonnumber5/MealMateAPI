export const groceryItemQueries = {
  ...
  getAllGroceryItemsByUser: `SELECT g.id, g.name, g.qty, m.measurement, g.isChecked FROM grocery_items g JOIN measurements m ON m.id = g.measurements_id WHERE g.users_id = ?;`,
  getGroceryItemById: `SELECT g.id, g.name, g.qty, g.measurements_id, g.isChecked FROM grocery_items g WHERE g.id = ? AND g.users_id = ?;`,

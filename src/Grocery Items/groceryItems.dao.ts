...
export const createGroceryItem = async (groceryItem: GroceryItem, userId: number) => {
    return execute<OkPacket>(groceryItemQueries.createGroceryItem, [groceryItem.name, groceryItem.mealId, groceryItem.qty, groceryItem.measurements_id, groceryItem.isChecked, userId]);
}
